...
if __name__ == '__main__':
    ...
    tsfiles = [f for f in glob.glob(os.path.join(
        args.tests, '*.txt')) if not os.path.basename(f).startswith('eval')]

    logging.info('Found {} groundtruths and {} test files.'.format(
    ...
    gt = OrderedDict([(Path(f).parts[-3], mm.io.loadtxt(f,
                     fmt=args.fmt, min_confidence=1)) for f in gtfiles])
    ...
    for k in change_fmt_list:
        fmt[k] = fmt['mota']
    logging.info(mm.io.render_summary(
