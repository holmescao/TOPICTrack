...
for dataset_config in tests:
    ...
    if dataset_name == "MotChallenge2DBox":
        ...
        file_loc = os.path.join(
            "mot_challenge",
            dataset_config["BENCHMARK"] + "-" + dataset_config["SPLIT_TO_EVAL"],
        )
    ...
    test_data_loc = os.path.join(
        os.path.dirname(__file__), "..", "data", "tests", file_loc
    )

    for cls in classes:
        results = {
            seq: raw_results[dataset_name][tracker][seq][cls]
            for seq in raw_results[dataset_name][tracker].keys()
        }
        ...
        test_data = trackeval.utils.load_detail(
            os.path.join(test_data_loc, tracker, cls + "_detailed.csv")
        )
